...
subroutine COVAR(sgm)
	...
	integer :: i, j, k !, ii
	real(8) :: cc1, cc2, invtemp1, invtemp2 !, u(Kmp), v(Kmp)

	Cov(1:Kmp,1:Kmp) = 0.d0
	HTH(1:Kmp,1:Kmp) = 0.d0

	! (HT*H + alpha^2*GTG) の計算 どちらの行列も JSP * JSP
	...
	do i = 1, Jsp
		do j = 1, i
			...
			do k = 1, Jdata
				cc1 = cc1 + Z_Green(k,i) * Z_Green(k,j)
			end do

			do k = 1, Jcp
				cc2 = cc2 + Alpha * Alpha * Cs(k,i) * Cs(k,j)
			end do
	...
	!===========逆行列============
	! http://detail.chiebukuro.yahoo.co.jp/qa/question_detail/q14141235799
	!=======================
	...
	do k = 1, Jsp
		...
		if(dabs(invtemp1) < 1.d-12) stop "!!WARNING!! in COVAR "
		! invtemp1 が小さいときは，逆行列が計算できないことに注意

		do j = 1, Jsp
			Cov(k,j) = Cov(k,j)/invtemp1
		end do
		...
		do i = 1, Jsp
			...
			if(i /= k) then
				...
				do j = 1, Jsp
					Cov(i,j) = Cov(i,j) - invtemp2 * Cov(k,j)
				end do
...
subroutine OUTSOL(hd,flobspd,nsource)
	...
	write(22,'(3(E10.4,1X),1X,3(I5,1X))') Alpha, Rsq2, Sigma, Jsp, Jcp, Jdata
